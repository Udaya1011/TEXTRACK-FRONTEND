...
import { getProductionStats, getStockOutStats, getProducts, getCategories, getBaseURL, getProductImage } from '../api/api';
                  ...
                  <tbody className="divide-y divide-dark-600/50 text-sm text-gray-300 font-medium">
                    ...
                    {type === 'designs' && rows.map((r) => (
                      <tr key={r.id} className="hover:bg-dark-600/30 transition-colors">
                        <td className="py-3 px-4">
                          {r.image ? (
                            <img src={getProductImage(r.image)} alt="" className="w-9 h-9 rounded-lg object-cover border border-dark-600" />
                          ) : (
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          {type === 'designs' && (
                            <div className="w-10 h-10 rounded-lg overflow-hidden bg-dark-800 shrink-0 border border-dark-600">
                              {r.image ? (
                                <img src={getProductImage(r.image)} alt="" className="w-full h-full object-cover" />
                              ) : (
