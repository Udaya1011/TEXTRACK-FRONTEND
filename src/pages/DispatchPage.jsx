...
import { getProducts, addStockOut, getStockOuts, deleteStockOut, getBaseURL, getProductImage } from '../api/api';
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-dark-800 rounded-2xl border border-dark-400 w-full max-w-sm max-h-[90vh] overflow-y-auto animate-slide-up">
        ...
        <div className="bg-dark-900 border-b border-dark-600 p-4 flex items-center gap-3">
          {product.image ? (
            <img src={`${getBaseURL()}${product.image}`} alt={product.category} className="w-12 h-12 rounded-lg object-cover" />
          ) : (
              ...
              {products.map(p => (
                <button key={p._id} onClick={() => setSelectedProduct(p)} 
                  ...
                  <div className="aspect-square rounded-xl overflow-hidden bg-dark-600 w-full relative">
                    {p.image ? (
                      <img src={getProductImage(p.image)} alt={p.category} className="w-full h-full object-cover" />
                    ) : (
            ...
            <div className="space-y-3">
              {entries.map(e => (
                <div key={e._id} className="card-glow p-4 flex items-center gap-4">
                  {e.product?.image ? (
                    <img src={getProductImage(e.product.image)} alt="" className="w-12 h-12 rounded-xl object-cover flex-shrink-0" />
                  ) : (
